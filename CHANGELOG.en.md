...
### Changed
- `set_mode_and_setpoint(bool, float)` → `set_mode(bool)` — removed unused `temp_celsius` parameter (protocol forbids sending setpoint before CMD_ON)
- Replaced manual setpoint rounding with `lroundf()`
- Moved `CMD_*` arrays from header to implementation file
